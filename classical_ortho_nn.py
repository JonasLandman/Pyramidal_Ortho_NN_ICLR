...
class Network(object):
    ...
    def __init__(self, sizes, cost=CrossEntropyCost, epsilon=10**(-4), activation=SigmoidActivation):
...
#         self.biases = [np.random.randn(y, 1) for y in self.sizes[1:]]
        ...
        #### APPLY WEIGHTS ORTHOGONALISATION ####
    ...
    def orthogonalize_weights(self):
        ...
        #print('BEFORE / self.weights[0] :\n',self.weights[0])
        #epsilon = 10**(-1)
        for layer in range(len(self.weights)):
            U, s, V = np.linalg.svd(self.weights[layer], full_matrices=False) 
            s_bounded = np.copy(s)
            for i in range(len(s)): #for each singular value
                if s[i] > 1+self.epsilon: #if bigger than 1+espilon, reduce it 
                    s_bounded[i] = 1+self.epsilon
                if s[i] < 1/(1+self.epsilon): #if smalle than 1/1+espilon, increase it 
                    s_bounded[i] = 1/(1+self.epsilon)
            #reform with the new singular values
            self.weights[layer] = np.dot(U,np.dot(np.diag(s_bounded),V))
        #print('AFTER / self.weights[0] :\n',self.weights[0])
        #print('Ortho ? ->\n', np.round(self.weights[0].T @ self.weights[0],3))
